  ...
  return (
    <div style={{ padding: "24px", maxWidth: "500px", margin: "0 auto" }}>
      ...
      {resultado && resultado !== "Verificando..." && (
        ...
        >
          <p style={{ fontSize: "1.4rem", fontWeight: "bold", color, margin: 0 }}>{resultado}</p>

          {/* Detalle del cupón si fue canjeado */}
          {infoCupon && (
            <div style={{ marginTop: "12px", color: "#333" }}>
              <p>
                 Nivel: <strong>{infoCupon.nivel}</strong>
              </p>
              <p>
                 Descuento: <strong>{infoCupon.descuento}%</strong>
              </p>
              <p>Vencía: {new Date(infoCupon.expires_at).toLocaleDateString()}</p>
            </div>
          )}

          <button onClick={resetear} style={{ marginTop: "12px", padding: "8px 20px", cursor: "pointer" }}>
            Validar otro cupón
          </button>
        </div>
      )}
